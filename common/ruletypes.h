...
RULE_CATEGORY(PVP)
RULE_INT(PVP, MinLevel, 2, "PvP begins at level 2 by default.")
RULE_INT(PVP, Settings, 0, "Sets the PVP settings for the server, 1 = Rallos Zek RuleSet, 2 = Tallon/Vallon Zek Ruleset, 4 = Sullon Zek Ruleset, 6 = Discord Ruleset, anything above 6 is the Discord Ruleset without the no-drop restrictions removed. TODO: Edit IsAttackAllowed in Zone to accomodate for these rules.")
RULE_INT(PVP, LevelDifference, 4, "4 level range difference by default")
RULE_INT(PVP, MeleeMitigation, 67, "Melee damage is mitigated by this amount.")
RULE_INT(PVP, SpellMitigation, 67, "Spell damage are mitigated by this amount.")
RULE_INT(PVP, RangedMitigation, 80, "In PvP, ranged attacks (archery/throwing) is mitigated by this amount.")
RULE_REAL(PVP, LevelWeightMultiplierPercentage, 5.0, "PVP Point System Level Weight Multiplier percentage per level difference between the killer and the victim.")
RULE_INT(PVP, InfamyCap, 8, "PVP Point System Infamy Cap.")
RULE_INT(PVP, KillCountDays, 1, "PVP Point System Kill Count Days. Prevent attacker from obtaining PVP points from the same victim within the number of days.")
RULE_INT(PVP, KillCountDaysLimit, 1, "PVP Point System Kill Count Days. Prevent attacker from obtaining PVP points from the same victim within the number of days.")
RULE_INT(PVP, PointsTimer, 600, "PVP Point System Delay Timer.")
RULE_INT(PVP, RespawnManaPercent, 100, "How much mana to respawn with when died from PVP.")
RULE_BOOL(PVP, AttackTimerEnabled, true, "Enable the pvp attack timer to prevent clients from escaping death by environmental or PvE deaths.")
RULE_INT(PVP, AttackTimer, 60000, "How much time to pass (MS) before client is attacked that they are considered not in pvp combat.")
RULE_INT(PVP, AttackTimerAfterZoning, 10000, "Same behavior as AttackTimer, except this value is the time (MS) to apply upon zoning when attacked recently before zoning.")
RULE_REAL(PVP, PushMultiplier, 0.5, "")
RULE_INT(PVP, ShadowrestZoneID, 0, "The zone id number to spawn on PVP death. 0 = Disabled")
RULE_REAL(PVP, ShadowrestZoneX, -54.87, "The Shadowrest X location on PVP death.")
RULE_REAL(PVP, ShadowrestZoneY, 19.47, "The Shadowrest Y location on PVP death.")
RULE_REAL(PVP, ShadowrestZoneZ, 1.74, "The Shadowrest Z location on PVP death.")
RULE_REAL(PVP, ShadowrestZoneHeading, 86.2, "The Shadowrest heading on PVP death.")
RULE_INT(PVP, LootRequest, 0, "Can players loot item from corpses? 0 = Forbidden, 1 = Single, 2 = All, 3 = Defined")
RULE_INT(PVP, LootRequestDefined, 0, "List item id number can be looted from player corpse.")
RULE_INT(PVP, LootableEquipSlots, 0, "When looting a player's corpse, equipped items in slots listed here are lootable.")
RULE_BOOL(PVP, CanLootNoTrade, false, "Can players loot no trade items from player corpses?")
RULE_BOOL(PVP, CanLootContainer, false, "Can players loot bags/containers from player corpses?")
RULE_BOOL(PVP, CanLootCoin, false, "Can players loot coin from player corpses?")
RULE_BOOL(PVP, CanLootCoinAuto, false, "Allow auto loot of coin in PVP death (CanLootCoin must be true)")
RULE_BOOL(PVP, RedNameFilter, false, "If enabled, changes from default red name for all pvp flagged players to only show red for players who you can pvp")
RULE_INT(PVP, RezBoxTimer, 10000, "Prevent player from accepting a resurection box while under pvp attack. Time is in ms, 10000 = 10 seconds")
RULE_INT(PVP, CotHTimer, 10000, "Prevent player from a Call of the Hero while under pvp attack. Time is in ms, 10000 = 10 seconds")
RULE_INT(PVP, GFluxTimer, 4000, "Prevent player from having the Gravity Flux spell effect applied within this time limit. Time is in ms, 4000 = 4 seconds")
RULE_INT(PVP, BaseInfamy, 118, "The base amount of infamy that a character starts with.")
RULE_INT(PVP, AdditionalInfamyPerLevel, 18, "The amount of infamy gained per level.")
RULE_REAL(PVP, InfamyStealPercentage, 20.0, "Percentage of infamy stolen in a PVP kill")
RULE_REAL(PVP, PointsGeneratedFromKillerInfamyPercentage, 0.5, "Percentage of killer infamy that contributes toward generated PvP points")
RULE_REAL(PVP, PointsGeneratedFromVictimInfamyPercentage, 47.5, "Percentage of victim infamy that contributes toward generated PvP points")
RULE_INT(PVP, MinimumPointsOnKill, 6, "Minimum amount of points awarded to a PVP killer.")
RULE_INT(PVP, MaximumPointsOnKill, 1000, "Maximum amount of points awarded to a PVP killer.")
RULE_INT(PVP, PointsAwardedToVictim, 1, "Amount of points awarded to a PVP victim.")
RULE_BOOL(PVP, AllowIllusionsToModifyBodyType, false, "Allow illusions to change the bodytype of the player, i.e. Undead Nukes to land on Undead Race Illusions.")
RULE_INT(PVP, KillCreditFlatDamageBase, 5, "Base (defender level 1) flat amount of player damage to award kill credit to the last eligible attacker on suicide, mob death, groupmate killshot, or guildmate killshot.")
RULE_INT(PVP, KillCreditFlatDamagePerLevel, 5, "Flat amount of player damage (per defender level) to award kill credit to the last eligible attacker on suicide, mob death, groupmate killshot, or guildmate killshot.")
RULE_CATEGORY_END()
...
RULE_REAL ( RoT, PvPBaseDamageMultiplier, 1, "1 is default, multiplies the base weapon damage by this amount, increases melee damage.")
...
RULE_BOOL ( RoT, NoLockedDoors, true, "When enabled, no doors are locked... Including teleport doors.")
RULE_BOOL ( RoT, DoubleAttackFlurry, true, "Alters default flurry code to trigger off double attack instead of triple attack without a level check.")
RULE_BOOL ( RoT, DisableGMCorruption, false, "Stops GMs from functioning as players, no casting, attacking, etc.")
RULE_REAL ( RoT, PvPPercantageForKillshot, .1, ".1 is default, .01-1 is the range, 10% of player health to snag the kill shot in a suicide or pve death situation makes sense.")
RULE_BOOL ( RoT, UnmemSpellsOnDeath, true, "On death, unmem spells from spellbar.")
RULE_INT ( RoT, RogueDoubleBackstabLevel, 55, "What level do Rogues start double backstabbing? Default: 55")
RULE_CATEGORY_END()
...
RULE_INT ( Spells, SnareBreakCheckChance, 75, "Determines chance for a snare break check to occur each buff tick.")
